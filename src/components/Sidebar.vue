<template>
  ...
  >
    ...
    <v-menu offset-x>
      <template #activator="{ on }">
        <v-list-item dense>
          ...
          <!-- <v-list-item-icon>
       <v-icon id="settingsMenuBtn" color="grey lighten-2" v-on="on"> mdi-plus </v-icon>
          </v-list-item-icon> -->
          <v-list-item-icon>
        ...
        <v-list-item>
          <v-list-item-content>
            <v-btn class="accent" @click="createBudget()">
              ...
              <span class="white--text">New Budget</span>
            </v-btn>
          </v-list-item-content>
        </v-list-item>
      </v-list>
    ...
    <v-list dark dense class="text-left pt-0 sidebar">
      ...
      <v-list-item v-for="budget in allBudgetNames" :to="{ path: `/budget/${budget}` }" @click="changeBudget(budget)" id="accountsSidebarBtn">
        <v-list-item-icon >
    ...
    <template #append>
      <v-list dense>
        ...
        <v-list-item v-if="!mini && !user.loggedIn">
          <v-btn block class="accent" :to="{ path: '/connect' }">
...
export default {
  ...
  computed: {
    ...
    // },
    allBudgetNames() {
      return [...this.budgetRoots.map(bud=>bud.name)]
    },
  ...
  methods: {
    changeBudget(clickedBudgetName){
      this.selectedBudget = Object.values(this.budgetRootsMap).filter(bud => bud.name === clickedBudgetName)
      console.log(this.selectedBudget)
      this.loadSelectedBudget( this.selectedBudget[0].short_id)
      this.selectedBudgetName = this.selectedBudget[0].name

    },
    loadSelectedBudget(selectedBudgetToLoad) {
      console.log(this.selectedBudget)
      this.$store.dispatch('setSelectedBudgetID', selectedBudgetToLoad)
    },
    createBudget() {
